...
export function DealershipManageClient({ dealership }: { dealership: Dealership }) {
    ...
    const td = useTranslations("dealership.manage");
    ...
    const handleDeleteVehicle = async () => {
        ...
        formData.set("vehicleId", vehicleToDelete.id);
        await deleteDealershipVehicle(formData);
        toast.success(td("removeSuccess"));
        setIsDeleting(false);
        ...
        <div className="min-h-screen bg-[#F8FAFC]">
            <div className="container mx-auto px-4 md:px-6 py-6 pt-8 lg:pt-14 space-y-6">
                            ...
                            <div className="flex-1 min-w-0 pb-1 w-full md:w-auto">
                                ...
                                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm font-bold text-slate-500">
                                    ...
                                    <span className="flex items-center gap-2 hover:text-[#3D0066] transition-colors cursor-default">
                                        ...
                                        {dealership._count.vehicles} {td("listings")}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            ...
                            <Sheet>
                                ...
                                <SheetContent className="w-full sm:max-w-xl overflow-y-auto p-8">
                                    <SheetTitle className="text-2xl font-black text-slate-900 tracking-tight mb-8">{td("editTitle")}</SheetTitle>

                                    {showEditDealership ? (
                                        <form onSubmit={handleUpdateDealership} className="space-y-6">
                                            <div className="flex items-center justify-between mb-2">
                                                <p className="text-sm font-bold text-[#3D0066]">{td("editingMode")}</p>
                                        ...
                                        <div className="space-y-10">
                                            <div className="flex items-center justify-between pb-6 border-b border-slate-100">
                                                <div>
                                                    <p className="text-[10px] font-black uppercase tracking-widest text-[#3D0066]">{td("accountStatus")}</p>
                                                    <p className="text-sm font-bold text-slate-500">{td("merchantPartner")}</p>
                                                </div>
                                                <Button
                                                    variant="outline"
                                            ...
                                            <div className="grid gap-2">
                                                <InfoItem icon={Building2} label={td("legalEntity")} value={dealership.name} />
                                                <InfoItem icon={MapPin} label={td("operationalZone")} value={dealership.location} />
                                                <InfoItem icon={Clock} label={td("availability")} value={dealership.operatingHours || td("notEstablished")} />
                                                <InfoItem icon={Phone} label={td("directLine")} value={dealership.phone || td("noContact")} />
                                            </div>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between py-6">
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">{td("catalogTitle")}</h2>
                        <p className="mt-1 text-sm font-medium text-slate-500">
                            {td("catalogSubtitle", { count: dealership.vehicles.length, unit: dealership.vehicles.length === 1 ? 'listing' : 'listings' })}
                        </p>
                    </div>
                    <NewListingModal
                        title={td("addCarTitle")}
                        subtitle={td("addCarSubtitle", { name: dealership.name })}
                ...
                <AlertDialog open={!!vehicleToDelete} onOpenChange={(open) => { if (!open) setVehicleToDelete(null); }}>
                    <AlertDialogContent className="rounded-2xl p-8 max-w-md">
                        <AlertDialogHeader>
                            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-red-50 border-2 border-red-100">
                                <Trash2 className="h-6 w-6 text-red-500" />
                            </div>
                            <AlertDialogTitle className="text-center text-xl font-black text-slate-900 tracking-tight">
                                {td("removeTitle")}
                            </AlertDialogTitle>
                            <AlertDialogDescription className="text-center text-sm text-slate-500 font-medium leading-relaxed">
                                {td.rich("removeConfirm", {
                                    title: vehicleToDelete?.title || "",
                                    span: (chunks) => <span className="font-bold text-slate-700">{chunks}</span>
                                })}
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter className="mt-6 flex gap-3 sm:flex-row">
                            <AlertDialogCancel
                                disabled={isDeleting}
                                className="flex-1 rounded-xl py-5 font-black text-xs uppercase tracking-widest border-2 border-slate-200 hover:bg-slate-50 cursor-pointer"
                            >
                                {td("cancel")}
                            </AlertDialogCancel>
                            <Button
                                ...
                                disabled={isDeleting}
                                className="flex-1 rounded-xl py-5 font-black text-xs uppercase tracking-widest bg-red-500 hover:bg-red-600 text-white cursor-pointer"
                            >
                                {isDeleting ? (
                                    <>
                                        <Loader2 className="size-4 animate-spin mr-2" />
